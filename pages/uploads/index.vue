...
const onChange = () =>{ //this is for uploading of videos normally //
    fileDisplay.value = URL.createObjectURL(file.value.files[0]); //url//
    fileData.value = file.value.files[0];
}


const onDrop =(e)=>{ //THis is for dragging of videos//
    errorType.value=''
    file.value = e.dataTransfer.files[0];
    fileData.value = e.dataTransfer.files[0];

    let extension = file.value.name.substring(file.value.name.lastIndexOf('.') + 1); //to check for file extension//
    if(extension !== 'mp4'){
...
const discard = () =>{  //thisis to discard a video //
    file.value = null;
    ...
    <UploadLayout>
        <div class="w-full mt-[80px] mb-[40px] bg-white shadow-lg rounded-md py-6 md:px-10 px-4">
            <div>
                ...
                <div class="text-gray-400 mt-1">Upload a video to your account</div>
            </div>

            <div class="mt-8 md:flex gap-6">
                <label v-if="!fileDisplay" @drop.prevent ="onDrop" @dragover.prevent="" for="fileInput" class="md:mx-0 mx-auto mt-4 mb-6 flex flex-col items-center justify-center 
                w-full max-w-[260px] h-[470px] text-center p-3 border-2 border-dashed border-gray-300 rounded-lg hover:bg-gray-100 cursor-pointer">
                <Icon name="majesticons:cloud-upload" size="40" color="#b3b3b1" />
                
                <div class="mt-4 text-[17px]"> Select Video to upload </div>
                <div class="mt-1.5 text-gray-500 text-[13px]"> Or drag and drop a file here </div>
                ...
                <div v-else class="md:mx-0 mx-auto mt-4  md:mb-12 mb-16 flex items-center justify-center w-full 
                max-w-[260px] h-[540px] p-3 rounded-2xl cursor-pointer relative">
                    <div class="bg-black h-full w-full" />
                    <!-- <img class="absolute z-20 pointer-events-none" src="@/assets/images/mobile-case.png"/> the phone case am seeing is not big enough-->
                ...
                <div class="mt-4 mb-6">
                    <div class="flex bg-[#f8f8f8] py-4 px-6">
                        <div>
                            ...
                            <div class="text-semibold text-[15px] text-gray-400">You can quickly divide your videos into multiple part and remove the redundant part </div>
                        </div>
