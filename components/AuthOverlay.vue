...
<div id="Overlay" class="fixed flex items-center justify-center z-50 top-0 left-0 w-full h-full bg-black bg-opacity-50">
    <div class="relative bg-white w-full max-w-[470px] h-[90%] p-4 rounded-lg">
        ...
        <div class="absolute flex items-center justify-center py-5 left-0 bottom-0 border-t w-full">
            <span class="text-[14px] text-gray-600">Don't have an account yet ?</span>
            <button @click="$event=>isRegister = !isRegister" class="text-[14px] text-[#f02c56] font-semibold pl-1">
                <span v-if="isRegister">Sign up</span>
                <span v-else="isLogin">Login </span>
            </button>
        </div>
    </div>
</div>
